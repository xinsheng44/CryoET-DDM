...
def extract_and_save_patches(image_path, coords_path, patch_size, save_dir):
    """
...
# coords_directory = '/data/wxs/tomo_denoise/TiltSeriesDDM_v2/data/EMPAIR-10164/particle_coords'  # 坐标文件目录
# save_directory = '/data/wxs/tomo_denoise/TiltSeriesDDM_v2/data/EMPAIR-10164/particle_patches'  # 保存噪声块的目录

# EMPIAR-10651
image_directory = '/data/wxs/tomo/EMPIAR_10651_project/0006/k2dft20s_14apra0006_slices'  # 原始图像目录
coords_directory = '/data/wxs/tomo_denoise/TiltSeriesDDM_v2/data/EMPAIR-10651/particle_coords'  # 坐标文件目录
save_directory = '/data/wxs/tomo_denoise/TiltSeriesDDM_v2/data/EMPAIR-10651/particle_patches'  # 保存噪声块的目录
x_flip = False
y_flip = True
...
# 遍历coords目录中的所有文件并处理
for tomo_star in tqdm(sorted(glob(os.path.join(coords_directory, "*.tomostar"))), ncols=80):
    print(f"处理文件夹: {tomo_star}")
    tomo_star_particles = 0  # 该 tomostar 文件夹的颗粒数
    valid_particles_tomo_star = 0  # 该文件夹中符合要求的颗粒数

    for coords_path in sorted(glob(os.path.join(tomo_star, "*.txt"))):
        
        if "000" not in coords_path:
            continue
        
        # angle = os.path.basename(coords_path).split(".txt")[0].split("_")[-1]
        
        image_name = os.path.splitext(coords_path.split(os.sep)[-1])[0] + '.mrc'  # 使用相同的文件名，后缀为.mrc
        ...
        with open(coords_path, 'r') as f:
            ...
            tomo_star_particles += num_particles
        
        
        # 处理对应的MRC图像
        if os.path.exists(image_file):
            print(f"  正在处理文件: {coords_path}")
            valid_particles_in_file = extract_and_save_patches(image_file, coords_path, patch_size, save_directory)
            valid_particles_tomo_star += valid_particles_in_file
            valid_particles_total += valid_particles_in_file  # 累加符合要求的颗粒数
        else:
            print(f"  对应的图像文件不存在: {image_file}")
    
    print(f"  {tomo_star} 中的颗粒数: {tomo_star_particles}, 符合要求的颗粒数: {valid_particles_tomo_star}")
